...
import { Pagination, ReportTable, ReportTotals } from "./report-parts";

const paymentRepository = new PrismaPaymentRepository();

/** Rows per page on screen (print always includes every row). */
const PAGE_SIZE = 20;
...
export default async function PaymentsReportPage({ searchParams }: PageProps) {
  ...
  const filters = parsePaymentsReportFilters(params);
  ...
  const pageRows = report.rows.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);
  ...
  return (
    <div dir={dir} className="mx-auto flex max-w-6xl flex-col gap-6">
      ...
      <form method="GET" className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 print:hidden">
        <Input name="q" defaultValue={filters.q} placeholder={t("searchPlaceholder")} className="h-9" />
      ...
      <div className="print:hidden">
        ...
        <Pagination
          ...
          query={params}
          label={t("pageOf", { page: safePage, total: totalPages })}
          prev={t("previous")}
          next={t("next")}
        />
      </div>
